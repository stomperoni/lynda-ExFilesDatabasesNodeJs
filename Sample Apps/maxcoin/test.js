...
function insertMongodb(collection, data) {
    ...
    Object.keys(data).forEach((key) => {
        promisedInserts.push(
            collection.insertOne({date: key, values: data[key]})
        )
    });
...
MongoClient.connect(dsn, (err, db) => {
    ...
    fetchFromAPI((err, data) => {
        ...
        .then((result) => {
            ...
            const options = {'sort': [['values', 'desc']]};
            collection.findOne({}, options, (err, doc) => {
                if(err) throw err;
                console.log(`MongodDB: The one month max value is ${doc.values} and it was reached on ${doc.date}`);
